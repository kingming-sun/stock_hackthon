...
export default function Report() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/analysis/${encodeURIComponent(symbol)}`, {
      method: 'POST',
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button className="px-3 py-1.5 rounded-md border border-border">分享</button>
          <button className="px-3 py-1.5 rounded-md border border-border">下载</button>
          <button className="px-3 py-1.5 rounded-md border border-border">收藏</button>
        </div>
      </div>
